...
async function initApp() {
    try {
        ...
        if (googleClientId) {
            // Initialise Google Identity Services once client ID is known
            google.accounts.id.initialize({
                client_id: googleClientId,
                callback: onGoogleCredential
            });
            document.getElementById('google-signin-btn').style.display = 'flex';
            document.querySelectorAll('.auth-divider').forEach(el => el.style.display = 'flex');
        } else {
            // Hide Google button if no client ID configured
            document.getElementById('google-signin-btn').style.display = 'none';
            document.querySelectorAll('.auth-divider').forEach(el => el.style.display = 'none');
        }
    } catch (e) {
        // Silently hide Google button if backend is unreachable at init
        document.getElementById('google-signin-btn').style.display = 'none';
        document.querySelectorAll('.auth-divider').forEach(el => el.style.display = 'none');
    }
...
function handleGoogleSignIn() {
    if (!googleClientId) {
        showAlert('alert-box',
            '⚠️ Google Sign-In is not configured yet. Open <code>backend/.env</code> and set your <strong>GOOGLE_CLIENT_ID</strong>, then restart the server.',
            'error'
        );
        return;
    }
...
async function onGoogleCredential(response) {
    try {
        const res  = await fetch(`${API}/auth/google`, {
            method: 'POST',
            headers: { 'Content-Type': 'application/json' },
            body: JSON.stringify({ id_token: response.credential, role: currentRole })
        });
        const data = await safeJson(res);
